...
4. Use OMP task completion/yield as the phase wait signal.
5. Run final validation.

Live feedback comes from OMP's native task/subagent cards. Main waits on OMP task results; `validate-phase.sh` validates after yield; `wait-phase.sh` is only for manual recovery, CI, or non-OMP automation.
...
Main spawns these agents in parallel:
...
Main lets OMP's subagent UI show live progress for all three parallel phases. As each phase task yields, Main summarizes its status and validates that phase:
...
Main runs `vulnops-intrusion` after triage.
...
Main runs `vulnops-reconcile` only after intrusion is terminal.
...
8. **No passive sleep polling.** Use OMP task/yield progress, then `scripts/validate-phase.sh`.
