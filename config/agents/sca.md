...
Write individual findings to `<scan_dir>/findings/<ecosystem>-<package>-<cve>.md` — these per-finding files are an agent analysis convenience and are NOT part of the validated contract (`validate-phase.sh` / `validate-scan.sh` do not require them). Downstream phases may read them when present:
