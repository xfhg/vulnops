...
if not selector:
    if not model_id:
        print("[bootstrap-omp] llm.selector and llm.model are empty", file=sys.stderr)
        raise SystemExit(1)
    selector = f"{provider_name}/{model_id}"

selector_provider, sep, selector_model = selector.partition("/")
if not sep or not selector_provider or not selector_model:
    print(f"[bootstrap-omp] invalid llm.selector: {selector!r}", file=sys.stderr)
    raise SystemExit(1)

custom_provider_selected = selector_provider == provider_name
...
config_lines.extend(f"  {role}: {q(selector)}" for role in role_names)
config_lines.extend(
    [
        "",
        "memory:",
...
model_name = str(model_record.get("name") or (model_id or selector_model).split("/")[-1])
...
if custom_provider_selected:
    ...
    models_lines.extend(
        [
            "    models:",
            f"      - id: {q(model_id or selector_model)}",
            f"        name: {q(model_name)}",
    ...
    models_lines = [
        "# Auto-generated by scripts/bootstrap-omp.sh. Do not edit.",
        "# Source of truth: config.toml.",
        f"# Model roles select {selector}; no custom provider registry is required.",
        "",
        "providers: {}",
        "",
    ]
...
write_if_changed(root / ".omp" / "config.yml", "\n".join(config_lines))
PY
