...
def main()->int:
    p=argparse.ArgumentParser();p.add_argument("--output",type=Path,required=True);p.add_argument("--receipt",type=Path,required=True);p.add_argument("inputs",type=Path,nargs="*");a=p.parse_args();started=now();advisories={};packages=0
