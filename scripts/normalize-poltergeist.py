...
def main() -> int:
    ...
    target=a.target.resolve(); candidates=[]
    for item in raw["results"]:
        ...
        candidates.append({"id":f"SEC-{stable}","type":str(item.get("rule_name") or rule)[:200],"rule_id":rule[:200],"classification":"candidate","file":rel,"line":line,"redaction":"<redacted>","exposure_path":"Matched in repository source; validity and runtime exposure require validation.","validity":"unknown"})
    warnings=[]
    for key in ("total_secrets_found","total_matches","matches"):
        expected=(raw.get("summary") or {}).get(key) if isinstance(raw.get("summary"),dict) else None
        if isinstance(expected,int) and expected != len(candidates): raise SystemExit("Poltergeist summary count does not match results")
    matches_found=(raw.get("summary") or {}).get("matches_found") if isinstance(raw.get("summary"),dict) else None
    if isinstance(matches_found,int):
        if matches_found < len(candidates): raise SystemExit("Poltergeist matches_found is lower than its result records")
        if matches_found != len(candidates): warnings.append(f"scanner reported {matches_found} matches represented by {len(candidates)} unique result records")
    doc={"schema_version":"2.0","tool":"poltergeist","candidate_count":len(candidates),"candidates":candidates}; data=write(a.output,doc)
    ...
    write(a.receipt,{"schema_version":"2.0","tool":"poltergeist","operation":"secret-scan","status":"ok","version":v[0] if v else "unknown","started_at":started,"completed_at":now(),"parse_status":"ok","result_count":len(candidates),"normalized_sha256":hashlib.sha256(data).hexdigest(),"warnings":warnings})
    return 0
