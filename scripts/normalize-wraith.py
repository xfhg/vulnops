...
def main() -> int:
    p=argparse.ArgumentParser(); p.add_argument("--repo", type=Path, required=True); p.add_argument("--lockfile", type=Path, required=True); p.add_argument("--output", type=Path, required=True); p.add_argument("--receipt", type=Path, required=True); a=p.parse_args()
    started=now()
    ...
    write(a.receipt,{"schema_version":"2.0","tool":"wraith","operation":"offline-scan","status":"ok","version":v[0] if v else "unknown","started_at":started,"completed_at":now(),"parse_status":"ok","result_count":len(advisories),"normalized_sha256":hashlib.sha256(data).hexdigest(),"warnings":[]})
    return 0
