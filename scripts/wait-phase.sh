...
try:
    ...
    sys.exit(1)
if manifest.get("status") != "ok" or plan.get("mode") != "targeted-ooda":
    sys.exit(1)
# Required-scope codegraph evidence: codegraph is the sole graph backend.
scopes = plan.get("scopes")
if not isinstance(scopes, list) or not scopes:
    sys.exit(1)
required_scopes = [s for s in scopes if isinstance(s, dict) and s.get("required")]
if not required_scopes:
    required_scopes = [s for s in scopes if isinstance(s, dict)]
for scope in required_scopes:
    sid = scope.get("id")
    if not sid:
        sys.exit(1)
    ctx = scan / "intrusion" / "codegraph-runs" / str(sid) / "codegraph-out" / "context.json"
    codegraph_ok = False
    if ctx.is_file():
        try:
        ...
        if isinstance(c, dict):
            codegraph_ok = (len(c.get("nodes", []) or []) + len(c.get("edges", []) or [])) > 0
    if not codegraph_ok:
        sys.exit(1)
sys.exit(0)
PY
