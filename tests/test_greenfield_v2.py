...
class ScannerContractTests(unittest.TestCase):
    ...
    def test_wraith_real_envelope_is_normalized_and_counts_are_checked(self) -> None:
        with tempfile.TemporaryDirectory(dir=ROOT / ".harness") as tmp:
            ...
            result = run(PYTHON, "scripts/normalize-wraith.py", "--repo", str(base), "--lockfile", str(base / "go.mod"), "--output", str(base / "out.json"), "--receipt", str(base / "receipt.json"), input_text=json.dumps(raw))
            ...
            bad = run(PYTHON, "scripts/normalize-wraith.py", "--repo", str(base), "--lockfile", str(base / "go.mod"), "--output", str(base / "bad.json"), "--receipt", str(base / "bad-receipt.json"), input_text=json.dumps(raw))
            self.assertNotEqual(bad.returncode, 0)
            empty = run(PYTHON, "scripts/normalize-wraith.py", "--repo", str(base), "--lockfile", str(base / "go.mod"), "--output", str(base / "empty.json"), "--receipt", str(base / "empty-receipt.json"), input_text=json.dumps({"package_count": 0, "vulnerability_count": 0, "results": None}))
            self.assertEqual(empty.returncode, 0, empty.stderr)

    def test_poltergeist_prelude_and_match_fields_never_survive(self) -> None:
        with tempfile.TemporaryDirectory(dir=ROOT / ".harness") as tmp:
            base = Path(tmp); target = base / "target"; target.mkdir(); (target / "app.py").write_text("placeholder\n")
...
class PrimitiveCampaignTests(unittest.TestCase):
    def fixture(self, base: Path) -> tuple[Path, Path]:
        ...
        write(scan / "sast/coverage-ledger.json", {"cells": [{"id": "CELL-1", "status": "shallow", "reason": "state transition not covered"}]})
    ...
    def test_zero_campaign_whole_scan_is_valid(self) -> None:
        ...
        with tempfile.TemporaryDirectory(dir=ROOT / "scans") as tmp:
            ...
            for tool,artifact_name,receipt_name in (("wraith","sca-advisories.json","wraith-receipt.json"),("poltergeist","secrets-redacted.json","poltergeist-receipt.json")):
                digest=hashlib.sha256((scan/"tool-collection"/artifact_name).read_bytes()).hexdigest();write(scan/"tool-collection"/receipt_name,{"schema_version":"2.0","tool":tool,"operation":"fixture","status":"ok","version":"fixture","started_at":"now","completed_at":"now","parse_status":"ok","result_count":0,"normalized_sha256":digest,"warnings":[]})
            write(scan/"tool-collection/collection.json",{"schema_version":"2.0","run_id":"run","sca_ref":"tool-collection/sca-advisories.json","secrets_ref":"tool-collection/secrets-redacted.json","receipts":["tool-collection/wraith-receipt.json","tool-collection/poltergeist-receipt.json"],"warnings":[]});(scan/"tool-collection/summary.md").write_text("# Tools\n");tool_outputs=["tool-collection/collection.json","tool-collection/sca-advisories.json","tool-collection/secrets-redacted.json","tool-collection/wraith-receipt.json","tool-collection/poltergeist-receipt.json","tool-collection/summary.md","tool-collection/phase-manifest.json"];write(scan/"tool-collection/phase-manifest.json",manifest("tool-collection",tool_outputs))
            threat={"schema_version":"2.0","repository_profile":{"kinds":["library"],"tags":[],"comparable":{"name":None,"basis":"fixture","confidence":"not_applicable"}},"assets":[{"id":"A-1","name":"library integrity","sensitivity":"medium","description":"correct behavior","evidence_refs":["app.py:1"]}],"trust_boundaries":[{"id":"TB-1","source_trust":"caller","target_trust":"library","description":"caller boundary","evidence_refs":["app.py:1"]}],"entrypoints":[{"id":"EP-1","path":"app.py","kind":"library","subsystem_ids":["SUB-1"],"trust_boundary_ids":["TB-1"],"evidence_refs":["app.py:1"]}],"subsystems":[{"id":"SUB-1","name":"library","files":["app.py"],"entrypoints":["EP-1"],"security_surface_ids":["EP-1"],"risk":"low","evidence_refs":["app.py:1"]}],"attack_classes":[{"id":"dependency","title":"Dependency review","domain":"general","owner":"sca","methodology_ref":"skill://vulnops-audit-core","applicable_subsystems":["SUB-1"],"reason":"tool owned","evidence_refs":["app.py:1"],"custom":False}],"threats":[{"id":"TH-1","title":"caller misuse","attacker":"caller","asset_ids":["A-1"],"entrypoint_ids":["EP-1"],"attack_class_ids":["dependency"],"description":"fixture threat","evidence_refs":["app.py:1"]}],"assumptions":[],"evidence_refs":["app.py:1"],"warnings":[],"errors":[]};write(scan/"sast/threat-model.json",threat)
            hunt={"schema_version":"2.0","run_id":"run","rationale":"fixture tool-owned coverage","budget":{"max_concurrency":1,"max_hunt_tasks":1,"max_gapfill_rounds":0,"max_attempts":1,"context_packet_bytes":1024},"custom_attack_classes":[],"cells":[{"id":"CELL-1","surface_id":"EP-1","subsystem":"SUB-1","attack_class_id":"dependency","domain":"general","status":"tool_satisfied","priority":"low","evidence_refs":["tool-collection/phase-manifest.json"],"owner":"sca","lead_key":None,"disposition_reason":"tool collected"}],"tasks":[],"warnings":[],"errors":[]};write(scan/"sast/hunt-plan.json",hunt)
